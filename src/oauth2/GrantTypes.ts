...
class GrantTypes {
  ...
  public static toArray(): GrantTypes[] {
    return [
...
class GrantTypesMiddleware {
  /*
    ...
    * */
  public static isValidGrantType(request, response, next: () => void) {
    try {
      assert.notEqual(
        response.locals.client.grantTypes.indexOf(response.locals.grantType),
        -1,
      )
      next()
    } catch (e) {
      const error = request.app
        .get('Oauth2Error')
        .find(Oauth2ErrorTypes.UnSupportedGrantType)
  ...
  /*
     * get body grant_type.
     * */
  public static getGrantType(request, response, next: () => void) {
    try {
  ...
  /*
     ...
     * */
  public static validateToNotBeImplicitGrantType(
    request,
    response,
    next: () => void,
  ) {
    try {
      ...
      const error = request.app
        .get('Oauth2Error')
        .find(Oauth2ErrorTypes.UnAuthorizedClient)
      response.status(error.statusCode).json(error.json)
    }
  }
  ...
  /*
     ...
     * */
  public static isAuthorizationCodeGrantType(
    request,
    response,
    next: () => void,
  ) {
  ...
  public static isRefreshTokenGrantType(request, response, next: () => void) {
    try {
