...
class TokenEndpoint {
  ...
  public static accessToken(request, response) {
    const $this: TokenEndpoint = TokenEndpoint.createInstance(request, response)
    /*
        * create callbacks
        * depend on request type this callbacks will evaluate and run after access_token create.
        * */
    let authorizationCodeGrantTypeCallback: () => void = () => {
      // empty block
    }
    let implicitGrantTypeCallback: () => void = () => {
      // empty block
    }
    let refreshTokenGrantTypeCallback: () => void = () => {
      // empty block
    }
    let clientCredentialsGrantTypeCallback: () => void = () => {
      // empty block
    }
