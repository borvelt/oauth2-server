...
class TokenIntrospection {
    ...
    private $introspectionResponse: any = {
        active: false,
        exp: null,
        scopes: null,
        token_type: null,
    ...
    public static introspect(request: Request, response: Response) {
        let $this: TokenIntrospection = TokenIntrospection.createInstance(request, response);
    ...
    private accessToken(next: Function) {
        let accessTokenResponse = this.$introspectionResponse;
        let result = this.$response.locals.accessToken;
        try {
    ...
    private refreshToken() {
        let refreshTokenResponse = this.$introspectionResponse;
        let result = this.$response.locals.refreshToken;
        try {
            assert.notEqual(result, null);
