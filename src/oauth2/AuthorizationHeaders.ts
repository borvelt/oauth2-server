...
class AuthorizationHeaderMiddleware {
    ...
    public static headerExists(request, response, next: Function) {
        let header: string = request.header('Authorization');
        try {
     ...
     * */
    public static basicAuthorizationRequired(request, response, next: Function) {
        try {
            let authorizationHeaderArray = response.locals.authorizationHeader.split(' ');
     ...
     * */
    public static bearerAuthorizationRequired(request, response, next: Function) {
        try {
            let authorizationHeaderArray = response.locals.authorizationHeader.split(' ');
     ...
     * */
    public static parseBasicAuthorizationHeader(request, response, next: Function) {
        let base64ClientCredential = response.locals.authorizationHeader.value;
        let base64Buffer: Buffer = new Buffer(base64ClientCredential, 'base64');
        let clientCredentialArray = base64Buffer.toString().split(':');
